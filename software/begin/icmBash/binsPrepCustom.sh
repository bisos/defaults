...
typeset RcsId="$Id: binsPrepCustom.sh,v 1.1 2020-08-24 05:05:20 lsipusr Exp $"
# *CopyLeft*
...
_CommentBegin_
*      ======[[elisp:(org-cycle)][Fold]]====== Apt-Pkg: something
_CommentEnd_
...
item_somethingCustom () {
  distFamilyGenerationHookRun binsPrep_somethingCustom
}

binsPrep_somethingCustom_DEFAULT_DEFAULT () {
    mmaThisPkgName="somethingCustom"
    ...
    typeset pkgRePubAgent="lcaPlone3PkgRePub.sh"

    function customInstallScript {
	opDo ${pkgRePubAgent} -h -v -n showRun -i prpBxObtain

	opDo sudo bash /var/osmt/distPkgs/all/VMware-Player-6.0.1-1379776.i386.bundle
    }
}
