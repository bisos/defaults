...
description: Use when reading, understanding, or considering modifying `py3/setup.py`, `py3/pyproject.toml`, `py3/_description.org`, or `py3/README.rst` in a bisos-pip package — all four are regenerated by `pypiProc.sh` and must NEVER be hand-edited. Also covers which scripts belong in the setup.py `scripts` list (only `.cs` and `.spcs`; never `.pcs`) and the mandatory COMEEGA dblock structure for new `.cs` files.
---

# bisos-pip `setup.py` and New CS File Conventions

## The four regenerated files — NEVER hand-edit any of them

Four files in every bisos-pip package's `py3/` directory are **regenerated
by `pypiProc.sh`** at release/reinstall time. Claude must not hand-edit any
of them — edits will be silently overwritten on the next `fullPrep` or
`pkgReInstall`.

| File                    | Regenerated by                                        | What to edit instead |
|-------------------------|-------------------------------------------------------|-----------------------|
| `py3/setup.py`          | `bx-dblock -i dblockUpdateFiles` (auto-detects contents of `bin/`, updates `scripts`, `requires`, `data_files`, etc.) | Nothing — the dblocks read the source tree directly. Add new `.cs` / `.spcs` files under `bin/`; they're picked up automatically. |
| `py3/pyproject.toml`    | Same dblock-driven regeneration                       | Nothing — same story as `setup.py`. |
| `py3/_description.org`  | `pypiProc.sh -i readmeToDescriptionOrg` copies `README.org` (repo root) and strips some org markups | `README.org` at the repo root. |
| `py3/README.rst`        | `pandoc --from=org -s -t rst` from `_description.org` | `README.org` at the repo root. |

**Why:** The developer owns the release toolchain
(`pypiProc.sh -v -n showRun -i pkgReInstall edit /bisos/venv/py3/bisos3`
and similar). Any hand-edit to these files creates diff churn that
vanishes on the next release step, and can silently break the build if
the edit conflicts with what the dblock regeneration expects.

**How to apply:**
- **Adding a new `.cs` / `.spcs` to `bin/`?** Just create the file. Do
  NOT edit `setup.py`'s `scripts` list — the `b:py3:pypi:setup/scripts`
  dblock auto-detects the file on the next `pypiProc.sh` run.
- **Updating the package description?** Edit `README.org` at the repo
  root (NOT `py3/_description.org`, NOT `py3/README.rst`). Between
  releases, `README.rst` being stale relative to `README.org` is
  expected and harmless.
- **Adding a new dependency?** The `requires` list is also
  auto-detected in some flows. If it needs manual guidance, put the
  guidance in `README.org` or in a comment ABOVE the `####+BEGIN:` line
  in `setup.py`, not inside the dblock body.

The rest of this skill covers `setup.py` in detail because it's the
file most commonly examined; the same "no hand-edits inside dblocks"
rule applies to `pyproject.toml` verbatim, and `_description.org` and
`README.rst` should be treated as build artefacts (not source).

## setup.py structure — for reference only, not for editing

`py3/setup.py` consists entirely of `####+BEGIN: b:py3:pypi:setup/<name>`
dblock sections. Each section's content between `####+BEGIN:` and
`####+END:` is generated by the `pypiProc.sh -i fullPrepBuild` pipeline.
